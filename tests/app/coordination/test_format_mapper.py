...
class TestFormatMapper(unittest.TestCase):
    ...
    def test_get_review_strategy_filename_valid(self):
        """Test review strategy filename retrieval for valid formats."""
        self.assertEqual(
            FormatMapper.get_review_strategy_filename('review-strategy'),
            'review_strategy.md'
        )
        self.assertEqual(
            FormatMapper.get_review_strategy_filename('review-strategy-branch'),
            'review_strategy_branch.md'
        )

    def test_get_review_strategy_filename_invalid(self):
        """Test review strategy filename retrieval for invalid formats."""
        self.assertIsNone(FormatMapper.get_review_strategy_filename('json'))
        self.assertIsNone(FormatMapper.get_review_strategy_filename('summary'))
        self.assertIsNone(FormatMapper.get_review_strategy_filename('unknown'))
