...
class TestCalculateOwnershipKPIs:
    ...
    @patch('src.app.file_processor.SharedOwnershipKPI')
    ...
    @patch('src.app.file_processor.SharedOwnershipKPI', side_effect=Exception('Git error'))
    @patch('src.app.file_processor.CodeOwnershipKPI', side_effect=Exception('Git error'))
    def test_calculate_ownership_handles_exceptions(self, mock_code, mock_shared):
        """Should use fallback KPIs on exceptions."""
        processor = FileProcessor(Mock(), {})
        
        with patch('src.app.file_processor.FallbackCodeOwnershipKPI') as fallback_code:
            with patch('src.app.file_processor.FallbackSharedOwnershipKPI') as fallback_shared:
