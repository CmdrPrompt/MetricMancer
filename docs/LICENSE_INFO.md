...
**Note**: Some packages (like `types-pytz`) may show as "UNKNOWN" in `pip-licenses` output due to the tool not fully
supporting the newer `License-Expression` metadata format. These packages have been manually verified to use permissive
licenses.
